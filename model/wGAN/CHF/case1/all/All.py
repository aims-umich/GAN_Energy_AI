...
def objective(trial):
    ...
    num_nodes = trial.suggest_int('num_nodes', 64, 256)
    ...
    class Discriminator(nn.Module):
        def __init__(self, input_size, num_layers, num_nodes):
            ...
            layers = [nn.Linear(input_size + 1, num_nodes), nn.ReLU()]  # Input + target concatenated
        ...
        def forward(self, x, y):
            ...
            return self.model(x)  # Wasserstein output
    ...
    test_y_pred = y_scaler.inverse_transform(pred_test_y)  # Inverse transform for predictions
...
print(f'Best all MAPE: {best_trial.value:.4f}')
