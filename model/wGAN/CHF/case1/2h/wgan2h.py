...
train_data = pd.read_csv("/home/unabila/WganCHF/2h/train_2h.csv").values
test_data = pd.read_csv("/home/unabila/WganCHF/all/test_all.csv").values
...
def objective(trial):
    ...
    lr_vae = trial.suggest_loguniform('lr_vae', 1e-4, 1e-2)
    lr_gan = trial.suggest_loguniform('lr_gan', 1e-4, 1e-2)
    num_epochs_vae = trial.suggest_int('num_epochs_vae', 100, 1000)
    num_epochs_gan = trial.suggest_int('num_epochs_gan', 250, 4000)
    ...
    num_nodes = trial.suggest_int('num_nodes', 64, 128)  # Number of nodes per layer
...
print(f'Best 5h hyperparameters: {best_trial.params}')
print(f'Best 5h MAPE: {best_trial.value}')

# Save the best predictions to CSV
#pd.DataFrame(best_test_y_pred, columns=["Predicted CHF"]).to_csv("best_predicted_chf_wgan.csv", index=False)
