...
for feature_combination in itertools.combinations(input_features, 2):
    ...
    y = new_data.iloc[:, 6:9].values  # Assuming columns 9, 10, 11 are the three parameters
    ...
    vae_model = VAE([6, 400, 400, 400, 10], latent_dim=10)  ###################################
    ...
    pd.DataFrame(y_test_pred).to_csv(f'pred_test_data_gan_{feature_combination[0]}_{feature_combination[1]}_removed.csv', index=False)
    ...
    for i, param in enumerate(["wind power", "solar power", "load power"]):
        ...
        plt.title(f'GAN prediction testing dataset ({param}) - {feature_combination[0]}_{feature_combination[1]}_removed mins')
        plt.ylabel(param, fontsize=20)
        plt.xlabel('Minutes', fontsize=20)
        plt.legend(loc='upper right')
        plt.savefig(f'prediction_wgan_{feature_combination[0]}_{feature_combination[1]}_removed_{param.replace(" ", "_")}.png')
...
results_df.to_csv('wgan12_6_feature_results.csv', index=False)
