...
for feature_to_remove in input_features:
    # Create a new DataFrame without the current input feature
    new_data = data_smoothed.drop(columns=[feature_to_remove])
    
    # Create a filename for the new file
    new_file_name = f'{feature_to_remove}_removed.csv'


    x = new_data.iloc[:, :7].values
    y = new_data.iloc[:, 7:10].values  # Assuming columns 9, 10, 11 are the three parameters
    ...
    vae_model = VAE([7, 400, 400, 400, 10], latent_dim=10)  ###################################
    ...
    pd.DataFrame(y_test_pred).to_csv(f'pred_test_data_wgan_{feature_to_remove}_removed.csv', index=False)
    ...
    for i, param in enumerate(["wind power", "solar power", "load power"]):
        ...
        plt.title(f'WGAN prediction testing dataset ({param}) - {feature_to_remove}_removed mins')
        plt.ylabel(param, fontsize=20)
        plt.xlabel('Minutes', fontsize=20)
        plt.legend(loc='upper right')
        plt.savefig(f'prediction_wgan_{feature_to_remove}_removed_{param.replace(" ", "_")}.png')
...
results_df.to_csv('wgan12h_7feature_results.csv', index=False)
