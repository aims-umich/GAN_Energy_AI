...
for i, train_size in enumerate(cases):
    ...
    pd.DataFrame(y_test_pred).to_csv(f'pred_test_data_wgan_train_size_{train_size}.csv', index=False)
    ...
    for i, param in enumerate(["wind power", "solar power", "load power"]):
        ...
        plt.title(f'WGAN prediction testing dataset ({param}) - train_size {train_size} mins')
        plt.ylabel(param, fontsize=20)
        plt.xlabel('Minutes', fontsize=20)
        plt.legend(loc='upper right')
        plt.savefig(f'prediction_wgan_train_size_{train_size}_{param.replace(" ", "_")}.png')
...
results_df.to_csv('wgan12h_train_size_results.csv', index=False)
