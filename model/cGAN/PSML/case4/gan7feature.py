...
for feature_to_remove in input_features:
    ...
    pd.DataFrame(y_test_pred).to_csv(f'pred_test_data_gan_{feature_to_remove}_removed.csv', index=False)
    ...
    for i, param in enumerate(["wind power", "solar power", "load power"]):
        ...
        plt.title(f'GAN prediction testing dataset ({param}) - {feature_to_remove}_removed mins')
        plt.ylabel(param, fontsize=20)
        plt.xlabel('Minutes', fontsize=20)
        plt.legend(loc='upper right')
        plt.savefig(f'prediction_gan_{feature_to_remove}_removed_{param.replace(" ", "_")}.png')
...
results_df.to_csv('gan12h_7feature_results.csv', index=False)
