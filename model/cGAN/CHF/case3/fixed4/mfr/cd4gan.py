...
def build_generator(z_dim, x_dim, num_layers, num_nodes):
    ...
    out_layer = Dense(2, activation='linear')(hidden)  # Output layer with 2 neurons for 2 output columns
...
def build_discriminator(x_dim, learning_rate, num_layers, num_nodes):
    x = Input(shape=(x_dim + 2,))  # Adjusted shape to include 2 output columns
...
def generate_y_test(generator, x_test, z_dim):
    ...
    # Assuming generator outputs two values, keep all rows and select only the last two columns
    return generated_y_test[:, -2:]
...
# Read train data
train_data = pd.read_csv("/home/unabila/CondGan/fixed4/mfr/xMass.csv").values
scaler = MinMaxScaler()
...
# Read test data
test_data = pd.read_csv("/home/unabila/CondGan/fixed4/test_mass.csv").values
Test = test_data
Y2 = test_data[:, -2]
Ytest = test_data[:, -1]
...
for item in configs:
    ...
    Y2n = generated_data[:, -2]

    mape = mean_absolute_percentage_error(Ytest, Ynn)
    r2 = r2_score(Ytest, Ynn)
    mape2 = mean_absolute_percentage_error(Y2, Y2n)
    r22 = r2_score(Y2, Y2n)
    MS_list.append((mape,r2,mape2,r22))
    ...
    #plot_comparison(Test,generated_data,4000,item) #dont forget to change
