...
def plot_diagonal(df1,df2, num_points_from_file2,item):
    ...
    plt.scatter(df1, df2, color='blue', label='Predicted vs True')

    # Plot a diagonal line for reference
    plt.plot([min(df1), max(df1)], [min(df2), max(df2)], color='red', linestyle='--', label='Ideal')

    # Add labels and title
    plt.xlabel('True CHF')
    plt.ylabel('Predicted CHF')
    plt.title(f'Diagonal Plot: True vs Predicted CHF (Config: {item})')
    plt.legend()
...
for item in configs:
    ...
    plot_comparison(test_data,generated_data,4000,item) 
    plot_diagonal(test_data,generated_data,4000,item)
