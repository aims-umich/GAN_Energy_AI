...
for feature_to_remove in input_features:
    ...
    pd.DataFrame(y_test_pred).to_csv(f'pred_test_data_gru_{feature_to_remove}_removed.csv', index=False)
    ...
    for i, param in enumerate(["wind power", "solar power", "load power"]):
        ...
        plt.savefig(f'prediction_gru_{feature_to_remove}_removed_{param.replace(" ", "_")}.png')
...
results_df.to_csv('gru_feature12h_to_remove_results.csv', index=False)
