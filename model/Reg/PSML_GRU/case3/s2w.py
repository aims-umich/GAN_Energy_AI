...
train_x, test_x = x[july:july + two_week, :], x[:ten_days, :]
train_y, test_y = y[july:july + two_week, :], y[:ten_days, :]
...
for lookback in lookback_values:
    ...
    pd.DataFrame(y_test_pred).to_csv(f'pred_test_data_s2w_{lookback}_lookback.csv', index=False)
    ...
    for i, param in enumerate(["wind power", "solar power", "load power"]):
        ...
        plt.savefig(f'prediction_gru_lookfd_s2w_{lookback}_{param.replace(" ", "_")}.png')
...
results_df.to_csv('gru_lookfor_s2w_results.csv', index=False)
